    ...
    for (int i = 0; i < m; i+=4)
    {
        for (int j = 0; j < n; j+=4)
        {
            for (int r = 0; r < k; r++)
            {
                ...
                C(i+1, j) += A(i+1, r) * B(r, j+0);
                C(i+1, j) += A(i+1, r) * B(r, j+1);
                C(i+1, j) += A(i+1, r) * B(r, j+2);
                C(i+1, j) += A(i+1, r) * B(r, j+3);

                C(i+2, j) += A(i+2, r) * B(r, j+0);
                C(i+2, j) += A(i+2, r) * B(r, j+1);
                C(i+2, j) += A(i+2, r) * B(r, j+2);
                C(i+2, j) += A(i+2, r) * B(r, j+3);

                C(i+3, j) += A(i+3, r) * B(r, j+0);
                C(i+3, j) += A(i+3, r) * B(r, j+1);
                C(i+3, j) += A(i+3, r) * B(r, j+2);
                C(i+3, j) += A(i+3, r) * B(r, j+3);
            }
        }
    }
}
