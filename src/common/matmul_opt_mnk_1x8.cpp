            ...
            float *c, int ldc)
{
    ...
    for (int i = 0; i < m; i++) // m*k
    {
        for (int j = 0; j < n; j += 8) // k*n
        {
            ...
            AddDot(k, &B(j + 4, 0), lda, &A(0, i), &C(i, j + 4));
            AddDot(k, &B(j + 5, 0), lda, &A(0, i), &C(i, j + 5));
            AddDot(k, &B(j + 6, 0), lda, &A(0, i), &C(i, j + 6));
            AddDot(k, &B(j + 7, 0), lda, &A(0, i), &C(i, j + 7));
        }
    }
}
